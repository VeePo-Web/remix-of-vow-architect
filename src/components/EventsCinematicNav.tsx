...
import { EVENTS_NAV_SECTIONS } from '@/config/eventsVideoActsConfig';

/**
 * EventsCinematicNav — Same architecture as CinematicNav (weddings)
 * but targets #events-cinema and uses events section config.
 */

const SECTION_BOUNDS = [
  { id: 'events-welcome',   start: 0.000, end: 0.125 },
  { id: 'events-atmosphere', start: 0.125, end: 0.250 },
  { id: 'events-occasions', start: 0.250, end: 0.375 },
  { id: 'events-approach',  start: 0.375, end: 0.500 },
  { id: 'events-threshold', start: 0.500, end: 0.625 },
  { id: 'events-proof',     start: 0.625, end: 0.750 },
  { id: 'events-offering',  start: 0.750, end: 0.875 },
  { id: 'events-crossing',  start: 0.875, end: 1.000 },
];

export function EventsCinematicNav() {
  ...
  return (
    <>
      ...
      <nav className="cn-dots" aria-label="Section navigation">
        ...
        {EVENTS_NAV_SECTIONS.map((section, i) => (
          <button
            key={section.id}
      ...
      <div ref={ctaRef} className="cn-cta-bar" style={{ opacity: 0, pointerEvents: 'none' }}>
        <Link to="/events/contact" className="cn-cta-btn">
          Discuss Your Event
        </Link>
      </div>
