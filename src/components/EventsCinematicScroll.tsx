 ...
 * but driven by the events config. Reuses the weddings video (76s),
 * one scroll zone, all text overlays at precise % positions.
...
function calcTextStyle(item: ScrollTextItem, progress: number) {
  ...
  // For items that hold past scroll 1.0, cap fade-in so they reach full brightness by 1.0
  const effectiveRange = item.exitAt > 1.0 ? Math.min(range, (1.0 - item.enterAt)) : range;
  const fadeIn = effectiveRange * 0.22;
...
export function EventsCinematicScroll() {
  ...
  return (
    <div
      ...
      ref={containerRef}
      className="video-act"
      style={{ height: EVENTS_SCROLL_HEIGHT }}
    >
      <div className="video-act__sticky">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${EVENTS_VIDEO_POSTER})`,
